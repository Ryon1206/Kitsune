...
const getAnimeDetails = async (animeId: string): Promise<IAnimeDetails> => {
  ...
  const seasons = (media.relations?.edges || []).map((edge) => ({
    id: String(edge.node.id),
  ...
  const recommendedAnimes = (media.recommendations?.nodes || []).map((rec) => {
  ...
  const relatedAnimes = (media.relations?.edges || []).map((edge) => {
